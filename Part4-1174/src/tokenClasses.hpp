...
public:
	...
	Token* process_declaration(){
		...
		{
			if((subTreeP = tokenObjectPs[tracker]->process_punctuator("*")))
			{
				if(!(lowerNodeP = assignment_exp()))
					return nullptr;
			...
			else if(( subTreeP = assignment_exp() ))
			{
				add_childP(subTreeP);
				return this;
			}
...
public:
	...
	Token* advance_past_conditional_operator() { return process_token();}
	void print_traverse()
	{
