...
public:
	...
	shared_ptr_to_constant eval()
	{
		auto assignment = this->stringValue[0];
		auto right = evalChildToken(1);
		auto key = getChildToken(0)->getStringValue();
		switch(assignment)
		{
			case '=': evalMap[key].sptc = evalMap[key].ts.ofTypeCreateConstant(right); break;
			case '-': evalMap[key].sptc = evalMap[key].sptc->minus(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
			case '+': evalMap[key].sptc = evalMap[key].sptc->add(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
			case '/': evalMap[key].sptc = evalMap[key].sptc->div(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
		}
...
public:
	...
	shared_ptr_to_constant eval()
	{
		...
		switch(assignment)
		{
			case '<': evalMap[key].sptc = evalMap[key].sptc->leftShift(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
			case '>': evalMap[key].sptc = evalMap[key].sptc->rightShift(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
			case '%': evalMap[key].sptc = evalMap[key].sptc->mod(*evalMap[key].ts.ofTypeCreateConstant(right)); break;
		}
		return evalMap[key].sptc;
	};
};
...
public:
	...
	shared_ptr_to_constant eval()
	{
		auto condition = evalChildToken(0)->conditional();
		return condition ? evalChildToken(1) : evalChildToken(2);
	}
};
...
public:
	...
	shared_ptr_to_constant eval()
	{
		switch(this->stringValue[0])
		{
			case '>': return evalChildToken(0)->rightShift(*evalChildToken(1)); break;
			case '<': return evalChildToken(0)->leftShift(*evalChildToken(1)); break;
		}
		return nullptr;
	}
};
...
public:
	...
	shared_ptr_to_constant eval()
	{
		...
		{
			case '-':
				return evalChildToken(0)->minus(*right);
			case '+':
				return evalChildToken(0)->add(*right);
